...
class A_Job_Posting(APIView):
    def get(self, request, Job_Posting_id):
        job_posting = Job_PostingSerializer(get_object_or_404(Job_Posting, id = Job_Posting_id))
        return Response(job_posting.data)
class Job_Postings_by_Skills(APIView):
    def get(self, request, job_skill):
        jobs = Job_PostingSerializer(Job_Posting.objects.all(), many = True)
        job_res = []
        for job in jobs:
            job_by_skill = job.skill.name.includes(job_skill)
            job_res.append(job_by_skill)
